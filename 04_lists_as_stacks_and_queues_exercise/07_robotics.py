...
robots = {}
for info in data:
    name, process_time = info.split("-")
    robots[name] = {"time": int(process_time), "free_at": None}
...
while products:
    ...
    for name, info in robots.items():
        if info["free_at"] is None or info["free_at"] <= time:
            free_robot = name
            break

    if free_robot:
        robots[free_robot]["free_at"] = time + timedelta(seconds=robots[free_robot]["time"])
        print(f"{free_robot} - {current_product} [{time.strftime('%H:%M:%S')}]")
    else:
